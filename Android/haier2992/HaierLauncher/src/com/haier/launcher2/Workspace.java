...
import com.haier.launcher2.FolderIcon.FolderRingAnimator;
import com.haier.launcher2.InstallWidgetReceiver.WidgetMimeTypeHandlerData;
...
public class Workspace extends SmoothPagedView
    ...
    //emirguo add 20120206
    private static final boolean mIsHomeFullScreen = true;
    ...
    void addInScreen(View child, long container, int screen, int x, int y, int spanX, int spanY,
        ...
        if (container == LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
            ...
            Log.d("zjf", "Workspace.addInScreen else ......");
    ...
    void delInScreen(View child, int screen){
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		System.out.println("!!!!!_Workspace.onTouch!!!!!_v:" + v);
    ...
    public boolean dispatchUnhandledMove(View focused, int direction) {
        ...
        if ((focused instanceof BubbleTextView)||(focused instanceof AppWidgetHostView)||(focused instanceof FolderIcon)){
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
    	...
    	System.out.println("#####_Workspace.onInterceptTouchEvent#####_ev.getAction():" + ev.getAction());
    ...
    protected void onPageBeginMoving() {
        ...
        mIsMoving = true;
        //mLauncher.ShowDefaultTvScreen();
        Log.d(TAG, "onPageBeginMoving call showTvBg()");
        mLauncher.showTvBg(true,true);
    ...
    protected void onPageEndMoving() {
        ...
        mIsMoving = false;
        mLauncher.HideDefaultTvScreen();
        if (getCurrentPage() == 1){//emirguo add for hide tv window,20120228
        	mLauncher.showTvWindow();
        } else {
        	CellLayout layout = (CellLayout) getChildAt(1);
        	SurfaceView tv_SurfaceView = (SurfaceView)layout.findViewById(R.id.Surface_view);
        	ImageView image_view = (ImageView)layout.findViewById(R.id.image_view);
        	System.out.println("Workspace.java, onPageEndMoving, tv_SurfaceView GONE");
        	tv_SurfaceView.setVisibility(View.GONE);
    		image_view.setVisibility(View.VISIBLE);
        }
        setDotBackground(getCurrentPage());
    }
    ...
    protected void setWallpaperDimension() {
        ...
        Log.i("Launcher", "mWallpaperHeight:" + mWallpaperHeight + "   mWallpaperWidth:" + mWallpaperWidth);
//zgzs
	    mWallpaperWidth = 1920;
        mWallpaperHeight = 1080;
        //System.out.println("wanglong---laucher----mWallpaperHeight:"+mWallpaperHeight+"   mWallpaperWidth:"+mWallpaperWidth);
		Log.i("Launcher", "mWallpaperHeight:" + mWallpaperHeight + "   mWallpaperWidth:" + mWallpaperWidth);
...
//zgzs 	fastInvalidate();
	...
	private void setDotBackground(int page) {
		
		System.out.println("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&_page:" + page);
		
		if(page == 0){
    		mLauncher.dot_1.setBackgroundResource(R.drawable.tips_red);
    		mLauncher.dot_2.setBackgroundResource(R.drawable.tips_gray);
    		mLauncher.dot_3.setBackgroundResource(R.drawable.tips_gray);
    	}
		if(page == 1){
      		mLauncher.dot_1.setBackgroundResource(R.drawable.tips_gray);
    		mLauncher.dot_2.setBackgroundResource(R.drawable.tips_red);
    		mLauncher.dot_3.setBackgroundResource(R.drawable.tips_gray);
    	}
		if(page == 2){
    		mLauncher.dot_1.setBackgroundResource(R.drawable.tips_gray);
    		mLauncher.dot_2.setBackgroundResource(R.drawable.tips_gray);
    		mLauncher.dot_3.setBackgroundResource(R.drawable.tips_red);
    	}
	}
...
//zgzs  		cl.setFastTranslationX(translationX);
            ...
            if (leftScreen != -1 && rightScreen != -1) {
                for (int i = leftScreen; i <= rightScreen; i++) {
                    ...
                    if (page.getVisibility() == VISIBLE &&
                            page.getAlpha() > 0.5f) {
//目前版本中没有setChildrenLayersEnabled，替代
							//   ((ViewGroup)getPageAt(i)).setChildrenLayersEnabled(true);
                         ((ViewGroup)getPageAt(i)).setClipChildren(true);
                    }
                }
            }
        }
 ...
 //zgzs                   ((ViewGroup)getChildAt(i)).setChildrenLayersEnabled(false);
 					((ViewGroup)getChildAt(i)).setClipChildren(true);
                }
            }
        }
    }
...
// cl.setFastScaleY(a * mOldScaleYs[i] + b * mNewScaleYs[i]);
            ...
            rotationAnim.addUpdateListener(new LauncherAnimatorUpdateListener() {
                public void onAnimationUpdate(float a, float b) {
                    ...
                    for (int i = 0; i < getChildCount(); i++) {
                        final CellLayout cl = (CellLayout) getChildAt(i);
//zgzs    cl.setFastRotationY(a * mOldRotationYs[i] + b * mNewRotationYs[i]);
...
//        final int outlineColor = getResources().getColor(android.R.color.holo_purple);
        final int outlineColor = getResources().getColor(R.color.haier_dark_background);
        Bitmap b;
...
//        final int outlineColor = r.getColor(android.R.color.holo_purple);
        final int outlineColor = r.getColor(R.color.haier_dark_background);
	...
	boolean createUserFolderIfNecessary(View newView, long container, CellLayout target, int[] targetCell,
			boolean external, DragView dragView, Runnable postAnimationRunnable) {
		
		Log.i("zhoujf", "Workspace.createUserFolderIfNecessary(...)......");
		...
		try {
			...
			Log.i("zhoujf", "Workspace.createUserFolderIfNecessary(...)_container:" + container + ",screen:" + screen
					+ ",cellX:" + targetCell[0] + ",cellY:" + targetCell[1]);
			if (screen == 1 && targetCell[0] == 3 && targetCell[1] == 1) {
				Log.i("zhoujf", "【所有应用】不与其它应用图标生成文件夹，请重新操作。");
				Toast.makeText(mLauncher, mLauncher.getResources().getString(R.string.show_all_app_not_folder),
			...
			Log.v("zjf", "Workspace.createUserFolderIfNecessary has error:" + e.getLocalizedMessage());
		}
		return false;
	}
...
//    	if (getCurrentPage() != 0){
//    		Object rawDragInfo = d.dragInfo;
//			if (!((rawDragInfo instanceof LauncherAppWidgetInfo) || (rawDragInfo instanceof ShortcutInfo)
//					|| (rawDragInfo instanceof FolderInfo))) {
//				snapToPage(0);
//			}
//    	}
        if (mDragTargetLayout != null) {
...
//            /*//emirguo closed because of haier801 only can add icon in home1 20120416
...
//            	if (getCurrentPage() == 0 || getCurrentPage() == 2){//emirguo add for only home1 can add folder,20120502
...
//            	}
    ...
    void moveToDefaultScreen(boolean animate) {
        if ((!isSmall()) && (getCurrentPage() != mDefaultPage)){
            if (animate) {
