...
import com.haier.launcher2.AppsCustomizePagedView.ContentType;
...
public class AppsCustomizeTabHost extends TabHost implements LauncherTransitionable,
    ...
    private boolean mFirstTabChange;
    private  LinearLayout mPageBar;

    public AppsCustomizeTabHost(Context context, AttributeSet attrs) {
        super(context, attrs);
        mLayoutInflater = LayoutInflater.from(context);
        mFirstTabChange = true;
    }
    ...
    protected void onFinishInflate() {
        ...
        mPageBar = (LinearLayout)findViewById(R.id.PageBar);
    }
    
    /*
    public static void setupPagePoint(Context context, int appPages,int widgetPages){
    	LayoutInflater infllater = LayoutInflater.from(context);
	    for (int i=0;i<appPages + widgetPages;i++){
	    	View PageView = infllater.inflate(R.layout.pagepoint, null);
	    	mPageBar.addView(PageView);
	    }
	    setupPagePointFocus(0);
	}*/
    
    // add by weijh 4/25/2012
    // delete static by hushuai 20140225 for launcher died
	public void updatePagePoint(Context context, int appPages, int widgetPages, int currPage) {
		
		try {	
			LayoutInflater infllater = LayoutInflater.from(context);
			int currNums = appPages + widgetPages;
			int pageNums = mPageBar.getChildCount();
			Log.d("kevin", "updatePagePoint, currNums = " + currNums);
			Log.d("kevin", "updatePagePoint, pageNums = " + pageNums);

			if (pageNums > currNums) {
				pageNums = pageNums - currNums;
				Log.d("kevin", "pageNums > currNums, pageNums = " + pageNums);
				for (int i = 0; i < pageNums; ++i) {
					View v = mPageBar.getChildAt(0);
					mPageBar.removeView(v);
				}
			} else if (pageNums < currNums) {
				pageNums = currNums - pageNums;
				Log.d("kevin", "pageNums < currNums, pageNums = " + pageNums);

				for (int i = 0; i < pageNums; i++) {
					View v = infllater.inflate(R.layout.pagepoint, null);
					mPageBar.addView(v);
				}
			}
			
			setupPagePointFocus(currPage);

		} catch (Exception e) {
			Log.d("kevin", "updatePagePoint has error：" + e.getLocalizedMessage());
		}
	}
    //delete static by hushuai 20140225 for launcher died
    public void setupPagePointFocus(int curPage){
    	if (null == mPageBar) {
    		return;
    	}
    	for(int i=0;i<mPageBar.getChildCount();i++){
    		View PageView = mPageBar.getChildAt(i);
    		ImageView imageView = (ImageView) PageView.findViewById(R.id.pageImage);
    		imageView.setBackgroundResource(R.drawable.tips_gray);
    	}
    	View PageView = mPageBar.getChildAt(curPage);
    	if (null == PageView) {
    		return;
    	}
    	ImageView imageView = (ImageView) PageView.findViewById(R.id.pageImage);
    	imageView.setBackgroundResource(R.drawable.tips_red);
    }
    ...
    private void reloadCurrentPage() {
        ...
        setupPagePointFocus(mAppsCustomizePane.getCurrentPage());
    }
        ...
        post(new Runnable() {
            ...
            public void run() {
                ...
                inAnim.addListener(new AnimatorListenerAdapter() {
                    @Override
                    public void onAnimationEnd(Animator animation) {
                    	//emirguo add for first enter allapp and click widget,the first can't get focus 20120406
                        if (mFirstTabChange && type == ContentType.Widgets){
                        	mAppsCustomizePane.invalidatePageData(-1, false);
                        	mFirstTabChange = false;
                        }
                        reloadCurrentPage();
                    }
                });
