...
//        mFocusedOutlineColor = mFocusedGlowColor = mPressedOutlineColor = mPressedGlowColor =
//        		res.getColor(android.R.color.holo_purple);
        //Home上图标周围的光晕
        mFocusedOutlineColor = mFocusedGlowColor = mPressedOutlineColor = mPressedGlowColor =
        		res.getColor(R.color.haier_dark_background);
...
//zgzs mLayout不知道哪里来的，注释掉
 /*                if (mLayout == null) {
                ...
                mStayPressed = false;
                setEllipsize(TruncateAt.MARQUEE);//emirguo add for marquee 20120410
                setCellLayoutPressedOrFocusedIcon();
            }else {
            	setEllipsize(TruncateAt.END);//emirguo add for marquee 20120410
            }
