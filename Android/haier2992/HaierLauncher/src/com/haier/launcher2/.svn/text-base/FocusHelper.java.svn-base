...
class IconKeyEventListener implements View.OnKeyListener {
	public boolean onKey(View v, int keyCode, KeyEvent event) {
		if ((event.getAction() == KeyEvent.ACTION_DOWN) && (keyCode != KeyEvent.KEYCODE_ENTER) && (keyCode != KeyEvent.KEYCODE_VOLUME_DOWN) && (keyCode != KeyEvent.KEYCODE_VOLUME_UP)){
			Launcher.playTone();
		}
...
public class FocusHelper {
	private static int Focus_X = 0;
	private static int Focus_Y = 1;
	private static int Focus_S = 1;
	...
	static boolean handlePagedViewGridLayoutWidgetKeyEvent(PagedViewWidget w, int keyCode, KeyEvent e) {
		...
		case KeyEvent.KEYCODE_TV_INPUT:
			...
			wasHandled = true;
			break;
//zgzs没有KEYCODE_HAIER_USB，以随意值替代
		case KeyEvent.KEYCODE_HAIER_USB:
		case KeyEvent.KEYCODE_HEADSETHOOK:
			if (Launcher.m_tvIsSmallWindows){
				Launcher.tv_DefaultScreen.setVisibility(View.VISIBLE);
				Launcher.m_tvNeedDelayChangeSource = false;
				mLauncher.startMyFunRootActivity();
			}
			wasHandled = true;
			break;
		default:
	...
	/**
	 * Private helper method to sort all the CellLayout children in order of their (x,y) spatially from top left to
	 * bottom right.
	 */
	 ...
	 * 
	 * @param delta
	 *            either -1 or 1 depending on the direction we want to search
	 */
	static View findCurrFocusViews(CellLayout layout, ViewGroup parent) {	
		...
		while (index < count) {
			...
			ItemInfo newInfo = (ItemInfo)newV.getTag();
			if (newV instanceof BubbleTextView || newV instanceof FolderIcon || newV instanceof AppWidgetHostView) {
				for (int row=newInfo.cellX;row<(newInfo.cellX+newInfo.spanX);row++){
					for(int line=newInfo.cellY;line<(newInfo.cellY+newInfo.spanY);line++){
						if ((line == Focus_Y) && (row == Focus_X)){
							return newV;
						}
					}
				}
	...
	private static View findViews(ArrayList<View> views, int i, int delta) {
		final int count = views.size();
		if (count <= 0){
			return null;
		}
		ItemInfo curInfo = null;
		if (i >= 0){
			curInfo = (ItemInfo)views.get(i).getTag();
		}else {
			curInfo = new ItemInfo();
			Focus_S = curInfo.screen = ((ItemInfo)views.get(0).getTag()).screen;
			if (delta == 1){
				Focus_X = -1;
				if (curInfo.screen == 1){
					Focus_Y = 1;
			...
			curInfo.cellX = Focus_X;
			curInfo.cellY = Focus_Y;
			curInfo.spanX = 1;
			curInfo.spanY = 1;
...
//	zhf - 121026	if (curInfo.screen == 1){
//			int[]t = new int[]{-1,-1,-1,-1,-1,-1,-1,-1,-1};
//			a[0] = t;
//		}
		/*
		for(int []x:a){
		...
		if (delta == 1){
			for(int line=Focus_Y;line<5;line++){
				for(int row=((line==Focus_Y)?(Focus_X+1):0);row<9;row++){
					if ((a[line][row] != -1)&&(a[line][row] != i)){
						Focus_X = row;
						Focus_Y = line;
						return views.get(a[line][row]);
					}
				}
			}
		}else if (delta == -1){
			for(int line=Focus_Y;line>=0;line--){
				for(int row=((line==Focus_Y)?(Focus_X-1):8);row>=0;row--){
					if ((a[line][row] != -1)&&(a[line][row] != i)){
						Focus_X = row;
						Focus_Y = line;
						return views.get(a[line][row]);
		...
		if (delta == 1){
			if ((curInfo.cellX + curInfo.spanX >= 9) && curInfo.screen != 2){
				return null;
			}
			for(int line=Focus_Y;line<curInfo.cellY+curInfo.spanY;line++){
				for(int row=curInfo.cellX+curInfo.spanX;row<9;row++){
					if (a[line][row] != -1){
						if ((curInfo.screen == 1 && curInfo.cellX == 0 && curInfo.cellY == 1 && line == 2 && row > 5)
								|| (curInfo.screen == 1 && curInfo.cellX == 0 && curInfo.cellY == 3 && line == Focus_Y && row > 5)){
							break;
						}else {
							Focus_X = row;
							Focus_Y = line;
							Focus_S = curInfo.screen;
							return views.get(a[line][row]);
						}
					}
				}
			}
			for(int line=Focus_Y-1;line>=curInfo.cellY;line--){
				for(int row=curInfo.cellX+curInfo.spanX;row<9;row++){
					if (a[line][row] != -1){
						Focus_X = row;
						Focus_Y = line;
			...
			for(int line=curInfo.cellY+curInfo.spanY;line<5;line++){
			...
			if ((curInfo.cellX == 0) && curInfo.screen != 0){
				return null;
			}
			for(int line=Focus_Y;line>=curInfo.cellY;line--){
				for(int row=curInfo.cellX-1;row>=0;row--){
					if (a[line][row] != -1){
						if ((curInfo.screen == 1 && curInfo.cellX == 6 && curInfo.cellY == 1 && line == 2 && row < 3)
								|| (curInfo.screen == 1 && curInfo.cellX == 6 && curInfo.cellY == 3 && line == Focus_Y && row < 3)){
							break;
						}else {
							Focus_X = row;
							Focus_Y = line;
			...
			for(int line=Focus_Y+1;line<curInfo.cellY+curInfo.spanY;line++){
				for(int row=curInfo.cellX-1;row>=0;row--){
					if (a[line][row] != -1){
						Focus_X = row;
						Focus_Y = line;
			...
			for(int line=curInfo.cellY-1;line>=0;line--){
			...
			for (int row=Focus_X;row<curInfo.cellX+curInfo.spanX;row++){
				for(int line=Focus_Y+1;line<5;line++){
					if ((a[line][row] != -1)&&(a[line][row] != i)){
						Focus_X = row;
						Focus_Y = line;
						Focus_S = curInfo.screen;
			...
			for(int row=Focus_X;row>=curInfo.cellX;row--){
				for(int line=Focus_Y-1;line>=0;line--){
					if ((a[line][row] != -1)&&(a[line][row] != i)){
						Focus_X = row;
						Focus_Y = line;
						Focus_S = curInfo.screen;
	...
	private static View findIndexOfIcon(ArrayList<View> views, int i, int delta) {
		...
		while (0 <= newI && newI < count) {
			...
			// emirguo add AppWidgetHostView for widget get focus 20120210
			if (newV instanceof BubbleTextView || newV instanceof FolderIcon || newV instanceof AppWidgetHostView) {
				return newV;
			}
			newI += delta;
	...
	private static View getIconInDirection(CellLayout layout, ViewGroup parent, View v, int delta) {
		final ArrayList<View> views = getCellLayoutChildrenSortedSpatially(layout, parent);
		System.out.println("getIconInDirection, views.size = " + views.size());
		return findIndexOfIcon(views, views.indexOf(v), delta);
	}

	private static View getIconInDirection(CellLayout layout, ViewGroup parent, int i, int delta, boolean Haierflag) {
		final ArrayList<View> views = getCellLayoutChildrenSortedSpatially(layout, parent);
		return findViews(views, i, delta);
	}

	private static View getIconInDirection(CellLayout layout, ViewGroup parent, View v, int delta, boolean Haierflag) {
		final ArrayList<View> views = getCellLayoutChildrenSortedSpatially(layout, parent);
		return findViews(views, views.indexOf(v), delta);
	}
	
	/**
	 * Private helper method to find the next closest BubbleTextView or FolderIcon in the direction delta on the next
	 * line.
	 * 
	 * @param delta
	 *            either -1 or 1 depending on the line and direction we want to search
	 */
	private static View getClosestIconOnLine(CellLayout layout, ViewGroup parent, View v, int lineDelta) {
		final ArrayList<View> views = getCellLayoutChildrenSortedSpatially(layout, parent);
		System.out.println("getClosestIconOnLine, views.size = " + views.size());
		...
		if (0 <= newRow && newRow < cellCountY) {
			...
			while (index != endIndex) {
				...
				boolean isWeather = ((tmpLp.cellY == 0)&&(((ItemInfo)newV.getTag()).screen == 1))?true:false;
				boolean satisfiesRow = (lineDelta < 0) ? (tmpLp.cellY < row) : (tmpLp.cellY > row);
				// emirguo add AppWidgetHostView for widget get focus 20120210
				if (satisfiesRow //&& (!isWeather)
						&& (newV instanceof BubbleTextView || newV instanceof FolderIcon || newV instanceof AppWidgetHostView)) {
	...
	private static void widgetChildrenFocused(View v){
    ...
    static boolean handleIconKeyEvent(View v, int keyCode, KeyEvent e) {
    	...
    	ItemInfo curInfo = (ItemInfo)v.getTag();
    	if (Focus_X >= curInfo.cellX && Focus_X < (curInfo.cellX+curInfo.spanX) 
    			&& Focus_Y >= curInfo.cellY && Focus_Y < (curInfo.cellY+curInfo.spanY) && curInfo.screen == Focus_S){
    	}else {
        	Focus_X = curInfo.cellX;
        	Focus_Y = curInfo.cellY;
        	Focus_S = curInfo.screen;
    	}
        ...
        switch (keyCode) {
            case KeyEvent.KEYCODE_DPAD_LEFT:
                if (handleKeyEvent) {
                	...
                	View newIcon = getIconInDirection(layout, parent, v, -1,true);
                    if (newIcon != null) {
                		newIcon.requestFocus();
                		if (newIcon instanceof AppWidgetHostView) {//emirguo add for widget don't have child, 20120316
                			widgetChildrenFocused(newIcon);
                    		
                		}
                    } else {
                        if (pageIndex > 0) {
                            parent = getCellLayoutChildrenForIndex(workspace, pageIndex - 1);
                            newIcon = getIconInDirection(layout, parent, -1, -1, true);
                            if (newIcon != null) {
                                newIcon.requestFocus();
                                if (newIcon instanceof AppWidgetHostView) {
                                	widgetChildrenFocused(newIcon);
                        		}
                            } else {
            ...
            case KeyEvent.KEYCODE_DPAD_UP:
                if (handleKeyEvent) {
                	...
                	View newIcon = getIconInDirection(layout, parent, v, -9, true);
                	if(newIcon == null){
                		newIcon = getClosestIconOnLine(layout, parent, v, -1);
                		if (newIcon != null){
                			ItemInfo info = (ItemInfo)newIcon.getTag();
                        	Focus_X = info.cellX;
                        	Focus_Y = info.cellY;
                		}
                	}
...
//                        tabs.requestFocus();
            ...
            case KeyEvent.KEYCODE_PAGE_UP:
                if (handleKeyEvent) {
                    ...
                    if (pageIndex > 0) {
                        parent = getCellLayoutChildrenForIndex(workspace, pageIndex - 1);
                        View newIcon = getIconInDirection(layout, parent, -1, 1, true);
                        if (newIcon != null) {
                            newIcon.requestFocus();
                            if (newIcon instanceof AppWidgetHostView) {
                            	widgetChildrenFocused(newIcon);
	...
	static boolean handleFolderKeyEvent(View v, int keyCode, KeyEvent e) {
		...
		FolderEditText title = folder.mFolderName;
		
		
		System.out.println("--FocusHelper.java -- hanlderFolderKeyEvent ");
		...
		case KeyEvent.KEYCODE_DPAD_RIGHT:
			if (handleKeyEvent) {
				...
				if (newIcon != null) {
					...
					title.requestFocus();
					System.out.println("v = " + v);
					System.out.println("newIcon = " + title);
				}
			}
			wasHandled = true;
			break;
	...
	public static void MoveToDefaultScreenWhenFirstScreenIsEmpty(Workspace workspace){
		CellLayoutChildren parent = getCellLayoutChildrenForIndex(workspace, 1);
	    View newIcon = getIconInDirection((CellLayout)workspace.getChildAt(0), parent, -1, 1, true);
	    if (newIcon != null) {
	        newIcon.requestFocus();
	        if (newIcon instanceof AppWidgetHostView) {
	        	widgetChildrenFocused(newIcon);
	...
	public static void PageUpWhenFocusWidgetChildren(Workspace workspace, CellLayout layout, int pageIndex){

        // Select the first icon on the previous page or the first icon on this page
        // if there is no previous page
        if (pageIndex > 0) {
        	CellLayoutChildren parent = getCellLayoutChildrenForIndex(workspace, pageIndex - 1);
            View newIcon = getIconInDirection(layout, parent, -1, 1, true);
            if (newIcon != null) {
                newIcon.requestFocus();
                if (newIcon instanceof AppWidgetHostView) {
	...
	public static void PageDownWhenFocusWidgetChildren(Workspace workspace, CellLayout layout, int pageIndex){
        // Select the first icon on the next page or the last icon on this page
        // if there is no previous page
        if (pageIndex < 2) {
        	CellLayoutChildren parent = getCellLayoutChildrenForIndex(workspace, pageIndex + 1);
