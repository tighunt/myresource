...
//zgzs
 //   private boolean mStartAllApps = false;
...
//        mFolderName.setKeyListener(null);
		...
		mFolderName.setOnTouchListener(new OnTouchListener() {

			@Override
			public boolean onTouch(View v, MotionEvent event) {
				int[] location = new int[2];
				mFolderName.getLocationOnScreen(location);
				Log.d(TAG, "mFolderName onTouch at y: " + location[1]);
				if (location[1] > 420) {
		...
		//        mFolderName.setImeOptions(EditorInfo.IME_FLAG_NO_FULLSCREEN);
    }
    ...
    public void onClick(View v) {
    	System.out.println("------Folder.java -- onClick----------");
        Object tag = v.getTag();
        System.out.println("-----Folder.onClick-----tag:" + tag);
        ...
        if (tag instanceof ShortcutInfo) {
            ...
            try
            {
	            if (item.intent.getComponent().getPackageName().equals("com.haier.showallapps")) {
	            	mLauncher.closeFolder();
//zgzs            	mStartAllApps = true;
					return ;
				}
            ...
            {
            	System.out.println("-----Folder.onClick-----Exception:" + e);
            }
...
//zgzs			Launcher.mstarTvChangeSource(packageName);
    ...
    public boolean onLongClick(View v) {
        ...
        if (tag instanceof ShortcutInfo) {
            ...
            if (!v.isInTouchMode()) {
             ...
             //默认所有应用不能移动，emirguo 20120227
            if (item.intent.getComponent().getPackageName().equals("com.haier.showallapps") 
            		&& (mInfo.screen == 1) && (mInfo.cellX == 3) && (mInfo.cellY == 1)){
            	boolean onlyOne = false;
            	for (ShortcutInfo info:mInfo.contents){
            		//只有一个所有应用时才不能移动
            		if (info.intent.getComponent().getPackageName().equals("com.haier.showallapps")){ 
            			if (onlyOne == true){
            				onlyOne = false;
            				break;
            			} else {
    ...
    public void doneEditingFolderName(boolean commit) {
        ...
        if (mLauncher.getWorkspace().getCurrentPage() == 1){
        	CellLayout child1 = (CellLayout) mLauncher.getWorkspace().getChildAt(1);
        	View tv_window = child1.findViewById(R.id.preview);
        	if (tv_window.getVisibility() == View.GONE){
        		SurfaceView tv_SurfaceView = (SurfaceView)child1.findViewById(R.id.Surface_view);
	        	ImageView image_view = (ImageView)child1.findViewById(R.id.image_view);
	        	System.out.println("Folder.java, doneEditingFolderName, tv_SurfaceView GONE");
	    		tv_SurfaceView.setVisibility(View.GONE);
	    		image_view.setVisibility(View.VISIBLE);
	    		child1.invalidate();
				mLauncher.setupTvSmallWindowBg(1200,true);
        	}
    	}
    }
...
//    	if (mLauncher.getWorkspace().getCurrentPage() == 1){
//    		mLauncher.showTvBg(false);
//    	}
		if (actionId == EditorInfo.IME_ACTION_UNSPECIFIED) {
			int[] location = new int[2];
			mFolderName.getLocationOnScreen(location);
			Log.d(TAG, "onEditorAction mFolderName location x: " + location[0] + " y: " + location[1]);
			if (location[1] > 420) {
 ...
 //               	mLauncher.showAllApps(true);
  //              }
	...
	private void replaceFolderWithFinalItem() {
		Log.i("zhoujf", "Folder--> replaceFolderWithFinalItem ");
		ItemInfo finalItem = null;

		if (getItemCount() == 1) {
			finalItem = mInfo.contents.get(0);
			Log.i("zhoujf", "finalItem:" + finalItem.container + "," + finalItem.cellX + "," + finalItem.cellY + ","
					+ finalItem.screen);
		}
		...
		if (finalItem != null) {
			Log.i("zhoujf", "addOrMoveItemInDatabase:" + mInfo.container + "," + mInfo.screen + "," + mInfo.cellX + ","
					+ mInfo.cellY);
			LauncherModel.addOrMoveItemInDatabase(mLauncher, finalItem, mInfo.container, mInfo.screen, mInfo.cellX,
					mInfo.cellY);
		}
		...
		if (finalItem != null) {
			...
			Log.i("zhoujf", "addInScreen:" + mInfo.container + "," + mInfo.screen + "," + mInfo.cellX + ","
					+ mInfo.cellY);
			mLauncher.getWorkspace().addInScreen(child, mInfo.container, mInfo.screen, mInfo.cellX, mInfo.cellY,
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        if (v == mFolderName && hasFocus) {
			...
			Launcher.tv_DefaultScreen.setVisibility(View.INVISIBLE);
		}
    }
}
