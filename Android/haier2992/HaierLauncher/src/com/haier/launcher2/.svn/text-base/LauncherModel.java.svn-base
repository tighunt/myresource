...
public class LauncherModel extends BroadcastReceiver {
	...
	static void addOrMoveItemInDatabase(Context context, ItemInfo item, long container, int screen, int cellX, int cellY) {
		Log.i("zhoujf", "addOrMoveItemInDatabase:" + item.container);
		if (item.container == ItemInfo.NO_ID) {
			// From all apps
			Log.i("zhoujf", "LauncherModel.addOrMoveItemInDatabase(...) begin......");
			addItemToDatabase(context, item, container, screen, cellX, cellY, false);
			Log.i("zhoujf", "LauncherModel.addOrMoveItemInDatabase(...) end......");
		} else {
			// From somewhere else
			Log.i("zhoujf", "LauncherModel.moveItemInDatabase(...) begin......");
			moveItemInDatabase(context, item, container, screen, cellX, cellY);
			Log.i("zhoujf", "LauncherModel.moveItemInDatabase(...) end......");
		}
	}
	...
	static void updateItemInDatabaseHelper(Context context, final ContentValues values, final ItemInfo item,
		...
		Runnable r = new Runnable() {
			public void run() {
				...
				Log.i("zhoujf", "LauncherModel.updateItemInDatabaseHelper_result:" + result);
				...
				if (item != modelItem) {
					...
					String msg = "item: " + ((item != null) ? item.toString() : "null") + "modelItem: "
							+ ((modelItem != null) ? modelItem.toString() : "null") + "Error: ItemInfo passed to "
							+ callingFunction + " doesn't match original";
     ...
     */
	static void moveItemInDatabase(Context context, final ItemInfo item, final long container, final int screen,
		...
		Log.i("zhoujf", "moveItemInDatabase:" + item.container + "," + item.cellX + "," + item.cellY + ","
				+ item.screen);
     ...
     */
	static void addItemToDatabase(Context context, final ItemInfo item, final long container, final int screen,
			final int cellX, final int cellY, final boolean notify) {

		Log.i("zhoujf", "LauncherModel.addItemToDatabase(...) begin......");

		item.container = container;
		item.cellX = cellX;
		item.cellY = cellY;

		Log.i("zhoujf", "addItemToDatabase container:" + container + ",cellX:" + cellX + ",cellY:" + cellY + ",screen:"
				+ screen);
		// We store hotseat items in canonical form which is this orientation invariant position
		// in the hotseat
		if (context instanceof Launcher && screen < 0 && container == LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
		...
		Log.i("zhoujf", "LauncherModel.addItemToDatabase(...) end......");
	}
    ...
    static void deleteItemFromDatabase(Context context, final ItemInfo item) {
        Log.i(TAG, "deleteItemFromDatabase:"+item.container+","+item.cellX+","+item.cellY);
        final ContentResolver cr = context.getContentResolver();
...
//                            mIconCache, mLabelCache));
                ...
                // hide haierTV and JRMservice app
				for (int j = 0; j < N; j++) {
					ResolveInfo shortcut = apps.get(j);
					String packageName = shortcut.activityInfo.packageName;
					if (Global.QUICKSEARCHBOX_PACKAGE_NAME.equals(packageName)
						|| Global.FLASH_PACKAGE_NAME.equals(packageName)
						|| Global.HAIERALLAPPS_PACKAGE_NAME.equals(packageName)
						){
						continue;
					} else 
					{
						mAllAppsList.add(new ApplicationInfo(packageManager, apps.get(j), mIconCache, mLabelCache));
					}
				}
            ...
            switch (mOp) {
                case OP_ADD:
                    for (int i=0; i<N; i++) {
                        ...
                        if (Global.FLASH_PACKAGE_NAME.equals(packages[i])
                                ){
                            continue;
                        }
    ...
    ShortcutInfo addShortcut(Context context, Intent data, long container, int screen,
		...
		Log.i("zhoujf", "LauncherModel.addShortcut(...) begin......");
		addItemToDatabase(context, info, container, screen, cellX, cellY, notify);
		Log.i("zhoujf", "LauncherModel.addShortcut(...) end......");
