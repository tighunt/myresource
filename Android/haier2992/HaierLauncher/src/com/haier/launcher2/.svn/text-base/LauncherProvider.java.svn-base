...
import com.haier.launcher2.LauncherSettings.Favorites;
...
public class LauncherProvider extends ContentProvider {
    ...
    static final String AUTHORITY = "com.haier.launcher2.settings";
	...
	public static Cursor queryForLauncher() {
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT * FROM ")
		   .append(TABLE_FAVORITES)
		   .append(" WHERE ")
		   .append(LauncherSettings.Favorites.SCREEN + " = 2");
		
		System.out.println("$$$$$$_LauncherProvider.queryForLauncher().sql:" + sql);

		SQLiteDatabase sqliteDatabase = mOpenHelper.getReadableDatabase();

		Cursor result = sqliteDatabase.rawQuery(sql.toString(), null);
    ...
    /**
     * Query "container" values in table
     * @return "container" values
     */
    public static List<Integer> queryContainers(){
        SQLiteDatabase sqliteDatabase = mOpenHelper.getReadableDatabase();
        Cursor cursor = sqliteDatabase.rawQuery("SELECT * FROM "+TABLE_FAVORITES, null);
        if(cursor == null){
            return null;
        }else{
            List<Integer> containerList = new ArrayList<Integer>();
            while(cursor.moveToNext()){
                containerList.add(cursor.getInt(cursor.getColumnIndex("container")));
            }
            cursor.close();
