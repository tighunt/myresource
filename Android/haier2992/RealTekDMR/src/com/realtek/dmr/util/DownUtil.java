...
public class DownUtil
{
	...
	public DownUtil(String url, String targetFile)
	{
		this.path = url;
	...
	public void download() throws Exception
	{
	    ...
	    if(targetFile.endsWith(".mp3"))
		{
			...
			int filenamelen = targetFile.length();
			String filename = targetFile.substring(0,filenamelen-5);
			RandomAccessFile file3 = new RandomAccessFile(filename+"b.mp3", "rw");
			file3.setLength(startdex+8192+256);
			file3.close();
			
			RandomAccessFile currentPart2 = new RandomAccessFile(filename+"b.mp3","rw");
		    currentPart2.seek(0);
			DownloadThread(0,startdex+8192,currentPart2);
			
			RandomAccessFile currentPart3 = new RandomAccessFile(filename+"b.mp3","rw");
			long filepart2 = currentPart3.length();	
			currentPart3.seek(filepart2-1);
			DownloadThread(fileSize-256,256,currentPart3);	
		}
	    else if(targetFile.endsWith(".wma"))
	    {
	    	RandomAccessFile wmaFile1 = new RandomAccessFile(targetFile,"rw");
	    	wmaFile1.setLength(256);
	    	wmaFile1.close();

	    	RandomAccessFile wmaFile2 = new RandomAccessFile(targetFile,"rw");
	    	wmaFile2.seek(0);
	    	DownloadThread(0,256,wmaFile2);
	    	
	    	RandomAccessFile wmaFile = new RandomAccessFile(targetFile,"r");
	    	wmaFile.seek(0);
	    	...
	    	RandomAccessFile wmaFile3 = new RandomAccessFile(targetFile,"rw");
	    	wmaFile3.setLength(startbyte+1024);
	    	wmaFile3.close();
	    	RandomAccessFile wmaFile4 = new RandomAccessFile(targetFile,"rw");
	    	wmaFile4.seek(0);
	    	DownloadThread(0,startbyte+1024,wmaFile4);
	    }
	    else if(targetFile.endsWith(".mp4"))
	    {	
	    	RandomAccessFile mp4File1 = new RandomAccessFile(targetFile,"rw");
	    	mp4File1.setLength(1024);
	    	mp4File1.seek(0);
	    	DownloadThread(0,1024,mp4File1);
	    	
	    	mp4Ftyp mp4ftyp = new mp4Ftyp(targetFile);
	    	int len1 = mp4ftyp.getlength();
	    	
	    	
	    	RandomAccessFile mp4File2 = new RandomAccessFile(targetFile,"rw");
	    	mp4File2.setLength(1024+len1);
	    	mp4File2.seek(0);
	    	DownloadThread(0,1024+len1,mp4File2);
	    	
	    	mp4Moov mp4moov = new mp4Moov(len1,targetFile);
	    	int len2 = mp4moov.getlength();
	    	
	    	RandomAccessFile mp4File3 = new RandomAccessFile(targetFile,"rw");
	    	long filelen = len2+len1+1024;
	    	mp4File3.setLength(filelen);
	    	mp4File3.seek(0);
	    	DownloadThread(0,filelen,mp4File3);
	    	
	    	RandomAccessFile mp4File4 = new RandomAccessFile(targetFile,"rw");
	    	mp4File4.seek(len1);
	    	...
	    	RandomAccessFile mp4File5 = new RandomAccessFile(targetFile,"rw");
	    	filelen = filelen+len3+2048;
	    ...
	    {	
	    	long length = 0;
	    	RandomAccessFile flacFile1 = new RandomAccessFile(targetFile,"rw");
	        ...
	        while (!isLastBlock)
	        {
		        ...
		        RandomAccessFile flacFile= new RandomAccessFile(targetFile,"rw");
		        flacFile.setLength(length+1024);
		        flacFile.seek(0);
			    DownloadThread(0,length+1024,flacFile);
			    mbh = null; //Free memory
	        }
	    }
	    else if(targetFile.endsWith(".ogg"))
	    {
	    	RandomAccessFile oggfile1= new RandomAccessFile(targetFile,"rw");
	        ...
	        if(pageHeader.getPageLength()>raf.length()-idexforpage)
            {
            	...
            	RandomAccessFile oggfile = new RandomAccessFile(targetFile,"rw");
	        	oggfile.setLength(1024+length);//cannot find the size of first pageheader
	        	oggfile.seek(0);
	        	DownloadThread(0,length+1024,oggfile);
            }
	        raf.close();
	        RandomAccessFile rafnext = new RandomAccessFile(targetFile,"rw");
        	...
        	rafnext = new RandomAccessFile(targetFile,"rw");
	        ...
	        while (rafnext.getFilePointer() >= 4)
	        {
	        	...
	        	if(pos < len)
	        	{
	        		...
	        		if(len3>fileSize)
	        			break;
	    	        RandomAccessFile oggfile2 = new RandomAccessFile(targetFile,"rw");
		...
		int length = 0;
			...
			URL url = new URL(path);
	          HttpURLConnection conn = (HttpURLConnection) url.openConnection();
				...
				while (!StopFlag && (length < currentPartSize)
