    len = strlen(tmp);
    LOGE("len= %d",len);
	char* pBufferData = new char[len+1];
	char* rechar = strcpy(pBufferData,tmp);
	rechar[len]='\0';
	//int reLen = fread(pBufferData, 1, len, file);
	//LOGE("relen= %d",reLen);
	hlsPlayList.SetContent(rechar,len);
	    // close the file
    if (file != NULL) {
        fclose(file);
        file = NULL;
...
static size_t HTTPData(void *buffer, size_t size, size_t nmemb, void *userData) {
	...
	if (buffer) {
		...
		canrestart = 0;
		lenth = write((unsigned char*)buffer, (unsigned int) len);
	}
	LOGE("lenth1=%d , lenth2= %d", len, lenth);
	return len;
}

void* curltest(void* param) {
	LOGE("curltest() funs");
	pageInfo_t page;
	CURL *curl;
	CURLcode res;
	...
	if (curl) {
		while (1) {
			...
			assert(0 == ok);
			PlayListItem* item = hlsPlayList.GetNextFile();
			//char* filename =
			if (item == NULL) {
				LOGE("item is null");
				canread == JNI_FALSE;
			...
			const char* filename = hlsPlayList.GetFileURL(item);
			LOGE("file http %s", filename);
...
char *jbyteArrayToByte(JNIEnv* env, jbyteArray data, int *outLen) {
	jbyte * olddata = (env)->GetByteArrayElements( data, JNI_FALSE);
...
jboolean getFile(jint n) {
	...
	LOGE("getfilestart1");
	LOGE("getfilestart2");
	cls = (env)->FindClass( "com/example/nativemedia/NativeMedia");
	LOGE("dddd");
	if (cls != 0) {

		mid = (env)->GetStaticMethodID( cls, "getFile", "(I)V");

		if (mid != 0) {

			(env)->CallStaticVoidMethod( cls, mid, n);
			return JNI_TRUE;
		} else
			return JNI_FALSE;
	} else
...
jboolean testCallback() {
	...
	//jboolean not;
    JNIEnv* pJniEnv = 0;
    if ((m_pJVM)->AttachCurrentThread(&pJniEnv, 0) < 0)
    {
	...
	LOGE("testCallback1");
	LOGE("testCallback2");
	cls = (pJniEnv)->FindClass( "com/example/nativemedia/NativeMedia");
...
static XAresult AndroidBufferQueueCallback(XAAndroidBufferQueueItf caller,
	...
	status = read((unsigned char*)pBufferData, &bytesRead, 188, 0);
...
static void StreamChangeCallback(XAStreamInformationItf caller,
		...
		void * pContext) {
	LOGE("StreamChangeCallback called for stream %u", streamIndex);
...
static jboolean enqueueInitialBuffers(jboolean discontinuity) {
	...
	int status = 0;
	//bytesRead = fread(pBufferData, 1, BUFFER_SIZE, file);
	status = read((unsigned char*)dataCache, &bytesRead, 188, 1);
...
JNIEXPORT jboolean JNICALL Java_org_apache_cordova_example_VideoPlayer_setFile(JNIEnv* env,
	...
	//LOGE("setfile");
	 ...
	 jclass clazzs = (env)->GetObjectClass( list);
	 jmethodID getMethodID = (env)->GetMethodID( clazzs, "get",
	 "(I)Ljava/lang/Object;");
	 jmethodID sizeMethodID = (env)->GetMethodID(clazzs, "size", "()I");
	 int size = (env)->CallIntMethod( list, sizeMethodID);
	 filenum = size;
	 ...
	 //{
	 jstring str = (jstring)(env)->CallObjectMethod( list, getMethodID,
	 i);
	 jboolean isCpopy;
	 char const * p = (env)->GetStringUTFChars(str, &isCpopy);
	 LOGE(p);
	 int pLen = strlen(p);
	 char* tmpChar = new char[pLen+1];
	 tmpChar[pLen]='\0';
	 memcpy(tmpChar,p,pLen);
	 files[i] = tmpChar;
	 LOGE(tmpChar);
	 //}
...
JNIEXPORT jboolean JNICALL  Java_org_apache_cordova_example_VideoPlayer_createStreamingMediaPlayer(
	...
	ret = pthread_create(&httpThd, NULL, curltest, NULL);
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void* reserved) {
	...
	assert(env != NULL);
	//m_ClassQueryMedia = (env)->FindClass( "com/example/nativemedia/NativeMedia");
