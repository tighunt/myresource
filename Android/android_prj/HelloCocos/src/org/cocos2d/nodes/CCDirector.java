...
import org.cocos2d.grid.CCGrabber;
import org.cocos2d.grid.CCGrabberRtk;
...
//        setProjection(projection_); is called in onSurfaceChanged()
    ...
    //xuli modify
    public CCTexture2D grabTexture[] = null;
    public CCGrabber grabber[] = null;
    public CCGrabberRtk grabberRtk = null;
    public static boolean mFboNeedUpdate[] = null;

    public static int fboWidth = 1280;
    public static int fboHeight = 720;
...
//		return;
    	...
    	/*if (grabTexture == null){    		
	        grabTexture = new CCTexture2D[4];
	        for (int i=0; i<4; ++i){
	        	Bitmap bitmap = Bitmap.createBitmap((int)fboWidth, (int)fboWidth, Config.ARGB_8888);
		        Canvas canvas = new Canvas(bitmap);
		        canvas.drawBitmap(bitmap, 0, 0, new Paint());
	        	grabTexture[i] = new CCTexture2D();
	        	grabTexture[i].initWithImage(bitmap, CGSize.make(fboWidth, fboWidth));
		        grabTexture[i].loadTexture(gl);	
	        }
    	}
    	if (grabber == null){
    		grabber = new CCGrabber[4];
    		for (int i=0; i<4; ++i){
    			grabber[i] = new CCGrabber();
    			grabber[i].grab(grabTexture[i]);
    		}
    	}*/
    	
    	if (grabberRtk == null){
    		grabberRtk = new CCGrabberRtk();
    	}
    	
    	if (mFboNeedUpdate == null){
    		mFboNeedUpdate = new boolean[4];
    		for (int i=0; i<4; ++i)
    			mFboNeedUpdate[i] = true;
    	}
...
//        }
    ...
    public void drawCCScene (GL10 gl) {
        ...
        drawCCScenePartToFbo(gl, 0);
        drawCCScenePartToFbo(gl, 1);
        drawCCScenePartToFbo(gl, 2);
        drawCCScenePartToFbo(gl, 3);
        
        /*drawCCScenePartToScreen(gl, 0);
        drawCCScenePartToScreen(gl, 1);
        drawCCScenePartToScreen(gl, 2);
        drawCCScenePartToScreen(gl, 3);
        */
        ...
        */        
        ...
        for (int i=0; i<4; ++i)
			mFboNeedUpdate[i] = false;
    }
            
    public void drawCCScenePartToFbo (GL10 gl, int part) {
    	if (mFboNeedUpdate[part] == false)
    		return;
    	
    	gl.glPushMatrix();
        gl.glTranslatef(-fboWidth * (part%2), -fboHeight * (1 - part/2), 0);        
        //grabber[part].beforeRender(grabTexture[part]);   
        grabberRtk.beforeRender(part);
        ...
        gl.glPushMatrix();
        if( displayFPS && (part==2))	//show fps on the left-bottom fbo
        	showFPS(gl);
                
        //grabber[part].afterRender(grabTexture[part]);
        grabberRtk.afterRender(part);
        gl.glPopMatrix();	
    }
  //xuli modify end
    
    /*public void drawCCScenePartToScreen (GL10 gl, int part) {
    	grabTexture[part].drawAtPoint(gl, CGPoint.ccp(fboWidth * (part%2), fboHeight * (part/2)));
    }*/
