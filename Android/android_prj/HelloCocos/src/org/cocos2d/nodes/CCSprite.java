...
//            vertexes.put(0);
  ...
  //xuli add
    ...
    public int partOfScreen(CGRect BB){
    	int part = 0;
    	float midScreenX = CCDirector.fboWidth;
    	float midScreenY = CCDirector.fboHeight;
    	CGSize partSize = CGSize.make(midScreenX, midScreenY);

    	CGRect part1 = CGRect.make(CGPoint.ccp(0, midScreenY), partSize);
    	CGRect part2 = CGRect.make(CGPoint.ccp(midScreenX, midScreenY), partSize);
    	CGRect part3 = CGRect.make(CGPoint.ccp(0, 0), partSize);
    	CGRect part4 = CGRect.make(CGPoint.ccp(midScreenX, 0), partSize);
    	    	
    	if (CGRect.intersects(BB, part1))
    		part |= 1;
    	if (CGRect.intersects(BB, part2))
    		part |= 2;
    	if (CGRect.intersects(BB, part3))
    		part |= 4;
    	if (CGRect.intersects(BB, part4))
    		part |= 8;
    ...
    private TransformValues oldTV = null;
    private int oldTextureID = 0;
    CGRect oldBB = new CGRect();
    
    public void preVisit() {
    	TransformValues newTV = getTransformValues();   
        if (oldTV == null){
        	oldTV = new TransformValues();
        	oldTV.rotation = 0;
        	oldTV.scale = CGPoint.ccp(1, 1);
        	oldTV.pos = CGPoint.ccp(0, 0);
        }                
        
        CGRect BB = getBoundingBox();
        if ( Math.abs(newTV.rotation - oldTV.rotation) > 0
        		|| CGPoint.ccpDistance(newTV.scale, oldTV.scale) > 0
        		|| CGPoint.ccpDistance(newTV.pos, oldTV.pos) > 0
        		|| texture_.name() != oldTextureID)
        {        	
        	int part = partOfScreen(BB);
        	if ((part&1) > 0)
        		CCDirector.mFboNeedUpdate[0] = true;
        	if ((part&2) > 0)
        		CCDirector.mFboNeedUpdate[1] = true;
        	if ((part&4) > 0)
        		CCDirector.mFboNeedUpdate[2] = true;
        	if ((part&8) > 0)
        		CCDirector.mFboNeedUpdate[3] = true;
        	
        	if (texture_.name() == oldTextureID){	//not the case of animation, old bounding box will be updated
	        	int oldPart = partOfScreen(oldBB);
	        	if ((oldPart&1) > 0)
	        		CCDirector.mFboNeedUpdate[0] = true;
	        	if ((oldPart&2) > 0)
	        		CCDirector.mFboNeedUpdate[1] = true;
        ...
        oldTV.rotation = newTV.rotation;
        oldTV.scale = CGPoint.make(newTV.scale.x, newTV.scale.y);
        oldTV.pos = CGPoint.make(newTV.pos.x, newTV.pos.y);
        oldTextureID = texture_.name();
        oldBB = CGRect.make(BB);
    }
    //xuli add end
