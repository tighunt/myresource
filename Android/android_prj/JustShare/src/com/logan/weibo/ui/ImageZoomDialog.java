...
public class ImageZoomDialog extends Activity implements OnTouchListener, OnClickListener {
	...
	private final String TAG="ImageZoomDialog";
	...
	private void initData() 
    {
		...
		new Thread(){
			public void run() {
				...
				Bitmap bmp = null;
		    	String filename = FileUtils.getFileName(img_url);
				try {
					//读取本地图片
					if(img_url.endsWith("portrait.gif") || StringUtils.isEmpty(img_url)){
						bmp = BitmapFactory.decodeResource(imgView.getResources(), R.drawable.widget_dface);
					}
					
					if(bmp == null){
						bmp = QImageUtil.getBitmapFromUrl(img_url);
						if(bmp != null){
							try {
		                    	//写图片缓存
...
//                (float) dm.heightPixels / (float) bitmap.getHeight());
	...
	private void enlarge() {
		try {
			...
			Matrix matrix = new Matrix();
			matrix.postScale(scaleWidth, scaleHeight);
			zoomedBMP = Bitmap.createBitmap(bitmap, 0, 0, bmpWidth, bmpHeight, matrix, true);
			imgView.setImageBitmap(zoomedBMP);
		} catch (Exception e) {
