...
class JNICallback: public MyCallback
{
public:
    JNICallback(JNIEnv* env, jobject thiz, jobject weak_thiz);
    ~JNICallback();
    virtual void writedata(char* data,int len);
private:
    JNICallback();
...
JNICallback::JNICallback(JNIEnv* env, jobject thiz, jobject weak_thiz)
{
...
void JNICallback::writedata(char* data,int len)
{
    /*JNIEnv *env = AndroidRuntime::getJNIEnv();
        jbyte *by = (jbyte*)data;
        jbyteArray jarray = env->NewByteArray(len);
        //env->SetByteArrayRegin(jarray, 0, len, by); 

        env->CallStaticVoidMethod(mClass, fields.writedata,
                jarray, len);
        ...
        XAuint32 itemsLength           /* input */)
{
    ...
    if (bytesRead > 0) {
        ...
        if (listener != NULL) {
        listener->writedata((char*)pBufferData,bufferSize);
        }
        assert(XA_RESULT_SUCCESS == res);
    } else {
...
static jboolean enqueueInitialBuffers(jboolean discontinuity)
{
    ...
    if (bytesRead <= 0) {
       ...
       if (listener != NULL) {
        listener->writedata(dataCache,bytesRead);
        }
    if ((bytesRead % MPEG2_TS_PACKET_SIZE) != 0) {
        LOGV("Dropping last packet because it is not whole");
    }
...
{
    jclass clazz;
    LOGE("try to get field writedata");
    ...
    env = jenv;
    fields.writedata = env->GetStaticMethodID(clazz, "writedata",
                                               "([BI)V");
    if (fields.writedata == NULL) {
        LOGE("fail to get field writedata");
        return;
    }
}
...
{
      listener = new JNICallback(env, thiz, weak_this);
}
