        ...
        XAuint32 itemsLength           /* input */)
{
    ...
    int bytesRead;
    bytesRead = Miracast_IO_read(pIOPlugin, pBufferData, BUFFER_SIZE, NULL);
...
static jboolean enqueueInitialBuffers(jboolean discontinuity)
{
    ...
    int bytesRead;
    int times = 0;
    bytesRead = Miracast_IO_read(pIOPlugin, dataCache, BUFFER_SIZE*NB_BUFFERS, NULL);
    		//fread(dataCache, 1, BUFFER_SIZE * NB_BUFFERS, file);
    /*if(bytesRead>0){
    	times ++;
    }
    while(bytesRead<0 && times<NB_BUFFERS){
    	LOGE("bytesRead %d bytes", bytesRead);
    	usleep(1000*10);
    	bytesRead = Miracast_IO_read(pIOPlugin, dataCache +times*BUFFER_SIZE, BUFFER_SIZE, NULL);
        if(bytesRead>0){
        	times ++;
        }
    }*/
