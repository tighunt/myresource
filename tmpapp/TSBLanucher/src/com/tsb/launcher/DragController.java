...
public class DragController {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP:
            	if((mDragObject == null || mDragObject.dragView == null) && Math.abs(mMotionDownX - dragLayerX)<100){//add quick launch
            		if(dragLayerY - mMotionDownY < - 100){
            			mLauncher.startMeidaPlayer();
            		}else if(dragLayerY - mMotionDownY>100){
            			mLauncher.startTv();
            		}
    ...
    private void handleMoveEvent(int x, int y) {
        ...
        //no Drag move effect on hotheat and fixed desk
        int page = mLauncher.getWorkspace().getNextPage();
        if(page == 2 )
        	mLauncher.showDropBar(false);
        else
        	mLauncher.showDropBar(true);
        if (dropTarget != null &&!(page == 2 && x>50 &&x<1870)&&y<=880) {
    ...
    private boolean drop(float x, float y) {
    	//if(y>880)
    		//return; // disable drop on hotseat
        final int[] coordinates = mCoordinatesTemp;
