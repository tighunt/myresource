...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    public PagedView(Context context, AttributeSet attrs, int defStyle,boolean isapp) {
        super(context, attrs, defStyle);
        isApp = isapp;
        mLauncher = (Launcher) context;
        TypedArray a = context.obtainStyledAttributes(attrs,
    ...
    protected boolean hitsPreviousPage(float x, float y) {
    	if(isApp)
    	{
    		...
    		return ((x < getRelativeChildOffset(mCurrentPage) - mPageSpacing)&&(y>24)&&(y<94));
    		//int h = getMeasuredHeight()/2;
    		//return ((x < getRelativeChildOffset(mCurrentPage) - mPageSpacing)&&(y>200)&&(y<h-100));
    	}
    	else
    ...
    protected boolean hitsNextPage(float x, float y) {
    	if(isApp)
    	{
			...
			int pagedownoffset = getResources().getInteger(R.integer.pagedown_offset);
    		return ((x < getRelativeChildOffset(mCurrentPage) - mPageSpacing)&&(y>getMeasuredHeight()-190+pagedownoffset)&&(y<getMeasuredHeight()-120+pagedownoffset));
    		//int h = getMeasuredHeight()/2;
    		//return ((x < getRelativeChildOffset(mCurrentPage) - mPageSpacing)&&(y>h+50)&&(y<getMeasuredHeight()-300));
    	}
    	else
    ...
    private void updateScrollingIndicatorPosition() {
        ...
        if(isApp)
        {
            ...
            int trackHegiht = pageHeight - mScrollIndicatorPaddingLeft - mScrollIndicatorPaddingRight;
            ...
            int indicatorSpace = trackHegiht / numPages;
            int indicatorPos = (int) (offset * (trackHegiht - indicatorSpace)) + mScrollIndicatorPaddingLeft;
			...
			int lastChildIndex = Math.max(0, getChildCount() - 1);
			int maxScrollX = getChildOffset(lastChildIndex)
					- getRelativeChildOffset(lastChildIndex);
			int trackWidth = pageWidth - mScrollIndicatorPaddingLeft
					- mScrollIndicatorPaddingRight;
			int indicatorWidth = mScrollIndicator.getMeasuredWidth()
					- mScrollIndicator.getPaddingLeft()
					- mScrollIndicator.getPaddingRight();

			float offset = Math.max(0f,
					Math.min(1f, (float) getScrollX() / maxScrollX));
			...
			if (hasElasticScrollIndicator()) {
				...
				int indicatorCenterOffset = indicatorSpace / 2 - indicatorWidth
						/ 2;
				indicatorPos += indicatorCenterOffset;
			}
