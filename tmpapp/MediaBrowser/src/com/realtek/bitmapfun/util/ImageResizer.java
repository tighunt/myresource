...
public class ImageResizer extends ImageWorker {
    ...
    private ImageCache mImageResizerCache = null;
            ...
            int imageWidth, int imageHeight)
    {
        ...
        mImageResizerCache = getImageCache();
    }
     ...
     *         requested width and height
     */
    public  synchronized Bitmap decodeSampledBitmapFromFile(String filename,
            int reqWidth, int reqHeight) {

        // First decode with inJustDecodeBounds=true to check dimensions
        final BitmapFactory.Options options = calculateImageSize(filename);
        PicSize ps =new PicSize(options.srcHeight == -1?0:options.srcHeight
        		,options.srcWidth== -1?0:options.srcWidth);
        if(mImageResizerCache == null)
        {
        	mImageResizerCache=getImageCache();
        }
        if (ps != null && mImageResizerCache != null) {
        	mImageResizerCache.addPicSizeToCache(filename, ps);
        }
       // mImageResizerCache.showsizecache(); //for debug
