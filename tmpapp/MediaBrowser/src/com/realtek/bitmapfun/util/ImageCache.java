...
public class ImageCache {
    ...
    private Map<String, PicSize> mSizeCache;
    ...
    private void init(Context context, ImageCacheParams cacheParams) {
        ...
        //set up size cache
        mSizeCache = new HashMap<String,PicSize>();
    }

    public int showsizecache()//for debug, add by hartley
    {
    	System.out.println("showsizecache"+mSizeCache.size());
    	for(Object ob:mSizeCache.keySet())
        {
        	System.out.println(ob
        			+" "
        			+mSizeCache.get(ob)
        			+"_"
        		    +mSizeCache.get(ob).getHeight()
        		    +"_"
        		    +mSizeCache.get(ob).getWidth());
        }
    	return mSizeCache.size();
    }
    public Map<String, PicSize> getsizemap()
    {
    	return mSizeCache;
    }
    ...
    public void addPicSizeToCache(String data, PicSize ps)
    {
		if(data == null || ps == null)
		{
			return;
		}
		 if(mSizeCache != null && !mSizeCache.containsKey(data))
		 {
			 mSizeCache.put(data, ps);
     ...
     * 
     * @param data Unique identifier for which item to get
     * @return The PicSize if found in cache, null otherwise
     */
    public PicSize getSizeFromMemCache(String data) {
        if (mSizeCache != null) {
            final PicSize memSize = mSizeCache.get(data);
            if (memSize != null) {
                return memSize;
            }
        }
