...
public abstract class ImageWorker {
    ...
    protected ImageWorker(Context context, LoadingControl loadingControl)
    {
        ...
        m_ResourceMgr = mContext.getResources();
    }
    ...
    public void loadImage(Object data, ImageView imageView, LoadingControl loadingControl, int type)
    {
    	...
    	else if(type == FileFilterType.DEVICE_FILE_DEVICE)
    	{
    		return;
    	}
        Bitmap bitmap = null;
    ...
    //hartley_zhu add
    public void loadImage(Object data, ImageView imageView, TextView exifText,LoadingControl loadingControl, int type)
    {
    	PicSize size = null;
    	//if the type of item is device or folder, there is no need to decode picture 
    	if(type == FileFilterType.DEVICE_FILE_DIR)
        ...
        if (mImageCache != null) {
            bitmap = mImageCache.getBitmapFromMemCache(String.valueOf(data));
            size = (PicSize)(mImageCache.getSizeFromMemCache(String.valueOf(data)));
        }
        String text_size = m_ResourceMgr.getString(R.string.size_exif);
        if(size != null)
        {
        	setSizeText(exifText,size,text_size);
        }
        if (bitmap != null) {
            // Bitmap found in memory cache
            imageView.setImageBitmap(bitmap);
            ...
            final BitmapWorkerTask task = new BitmapWorkerTask(imageView, loadingControl,exifText);
  ...
  //hartley add
    public void loadImage(Object data, ImageView imageView, TextView exifText,LoadingControl loadingControl,boolean isExif)
    {
    	Bitmap bitmap = null;
    	PicSize size = null;
        ...
        if (bitmap != null) {
            ...
            final BitmapWorkerTask task = new BitmapWorkerTask(imageView, loadingControl, isExif,exifText);
    ...
    //hartley add
    public void loadImage(Object data, ImageView imageView, LoadingControl loadingControl,boolean isExif)
    {
    	Bitmap bitmap = null;
    	PicSize size = null;
        ...
        if(size == null)
        {
	    	...
	    	try {
	    		exif = new ExifInterface(String.valueOf(data));
	    	} catch (IOException e) {
        	...
        	{
        		size = new PicSize(Integer.valueOf(length), Integer.valueOf(width));         		
        		mImageCache.addPicSizeToCache(String.valueOf(data),size);        
        		//mImageCache.showsizecache();//for debug
        	}	    	
        }
        ...
        if (bitmap != null) {
            ...
            final BitmapWorkerTask task = new BitmapWorkerTask(imageView, loadingControl, isExif);
    ...
    private class BitmapWorkerTask extends AsyncTask<Object, Void, Bitmap> {
        ...
        public BitmapWorkerTask(ImageView imageView, LoadingControl loadingControl,TextView exifTV)
        {
			mOptions = new BitmapFactory.Options();
			mOptions.inDither = false;
			mOptions.inPreferredConfig = Bitmap.Config.ARGB_8888;
        ...
        public BitmapWorkerTask(ImageView imageView, LoadingControl loadingControl,boolean isExif,TextView exifTV)
        {
			mOptions = new BitmapFactory.Options();
			mOptions.inDither = false;
			mOptions.inPreferredConfig = Bitmap.Config.ARGB_8888;
        ...
        public BitmapWorkerTask(ImageView imageView,LoadingControl loadingControl, boolean isExif) {
        ...
        protected void onPostExecute(Bitmap bitmap) {
            ...
            String text_size = m_ResourceMgr.getString(R.string.size_exif);
            if(data!=null)
            	pSize = mImageCache.getSizeFromMemCache(String.valueOf(data));
            if(pSize != null)
            { 
            	//check_if_another_size is used to check if another size of Pic has been set to the textview
            	//if set then do not change the textveiw content
            	//if not set ,the textview is "size:0X0",and should be changed.
            	String check_if_another_size =null;
            	if(tv !=null)
            		check_if_another_size = String.valueOf(tv.getText()); 

            	if(check_if_another_size != null)
            	{   
            		if(check_if_another_size.indexOf("0X0")!=-1)
            	    {
            	    	setSizeText(tv,pSize,text_size);
            	    }
            ...
            {
            	imageView.setBackgroundResource(R.drawable.others_icon_photo_bad);
	...
	*/
    ...
    public static synchronized void setSizeText(TextView tv, PicSize ps,String title)
    {
    	if(tv != null && ps != null)
    	{
    		tv.setText(title+ps.getWidth()+"X"+ps.getHeight());
    	}
    }
}
