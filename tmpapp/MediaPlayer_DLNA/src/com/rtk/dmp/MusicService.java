...
public class MusicService extends Service implements MediaPlayer.OnCompletionListener,Observer{
	...
	private boolean errorSta = false;
	...
	private final int MSG_PLAY_NEXT = 6;
	...
	private synchronized void init() {
		...
		try {
				...
				protolinfoList = DLNADataProvider.queryResourceListByIndex(musicList.get(index).getUniqueCharID(),DLNADataProvider.UPNP_DMP_RES_PROTOCOLINFO); 
				ArrayList<String> contentLenList = new ArrayList<String>(); 
				contentLenList = DLNADataProvider.queryResourceListByIndex(musicList.get(index).getUniqueCharID(),DLNADataProvider.UPNP_DMP_RES_SIZE); 
				ArrayList<String> durationList = new ArrayList<String>(); 
				durationList = DLNADataProvider.queryResourceListByIndex(musicList.get(index).getUniqueCharID(),DLNADataProvider.UPNP_DMP_RES_DURATION); 
	...
	private OnPreparedListener audioPreparedListener = new OnPreparedListener() {
		...
		public void onPrepared(MediaPlayer mp) {
			...
			boolean bDTCP = false;
			String tmpProtolinfo = "";
			tmpProtolinfo = DLNADataProvider.queryDataByID(musicList.get(index).getUniqueCharID(), DLNADataProvider.UPNP_DMP_RES_PROTOCOLINFO,null);
	...
	private OnErrorListener mp_error = new OnErrorListener() {
		public boolean onError(MediaPlayer mp, int what, int extra) {
			...
			Log.e("musucservice", "indext =" +index + "error number = " +what);
			musicList.get(index).setCanPlay(-1);
			isReset = true;
			if(!quitPlay)
			{
