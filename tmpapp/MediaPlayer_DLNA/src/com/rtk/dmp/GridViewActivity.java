...
public class GridViewActivity extends FragmentActivity implements UiListener,Observer {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mMediaApplicationMap.addObserver(this);
		if (m_PathInfo != null) {
		...
		mainView = (View)findViewById(R.id.mainMutliViewLayout);
	//	mainView.setSystemUiVisibility(View.SYSTEM_UI_FLAG_LAYOUT_STABLE
    //            | View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION);
		final boolean isNav = true;
		...
		/*mainView.setOnTouchListener(new OnTouchListener(){
			...
			//        GridViewActivity.this.getWindow().getDecorView().setSystemUiVisibility(newVis);
				return false;
			}
		...
		gestureScanner = new GestureDetector(this,new GestureDetector.SimpleOnGestureListener(){
			...
			public boolean onSingleTapConfirmed(MotionEvent e) {  
				System.out.println("onDoubleTappppp");
				int newVis = View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
		                | View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION
		                | View.SYSTEM_UI_FLAG_LAYOUT_STABLE;
		        if (!isNav) {
		...
		sortUp.setOnClickListener(new OnClickListener(){
			...
			public void onClick(View v) {
				...
				Collections.sort(listItems, new DLNAFileInfoComparatorWithMode(sortMode,sortOrder));
			...
			public void onClick(View v) {
				...
				else
				{
					mQuickMenu.showAtRTop(DensityUtil.dip2px(getApplicationContext(), 14),
							DensityUtil.dip2px(getApplicationContext(), 72),117);
...
/*	private int getSleepTimeValue()
 ...
 private void setTopMediaPathInfoTextView()
	...
	//				onBackClicked();
			...
			AndroidMessageBox.alertMessageOnly(this, getResources().getString(R.string.msg_noFile), 
					AndroidMessageBox.alertMessageOnly_type1, 0);
			return;
		}
	}
	...
	//				onBackClicked();
		...
		if (listItems.size() >= MediaApplication.MAXFILENUM) {
			...
			PopupMessageShow(
					...
					5000);*/
			AndroidMessageBox.alertMessageOnly(this, getResources().getString(R.string.maxfilenum), 
					AndroidMessageBox.alertMessageOnly_type1, 0);
		}
		...
		if (iDirSize > 0) {
			for (int i = 0; i < iDirSize; i++) {
				DLNAFileInfo finfo = new DLNAFileInfo(i,
						...
						DLNADataProvider.queryDataByID(
								DLNADataProvider.getDirectoryCharID(i),
		...
		if (iItemSize > 0) {
			for (int i = 0; i < iItemSize; i++) {
				...
				if(DLNADataProvider.GetMediaType(FileName) == FileFilterType.DEVICE_FILE_PHOTO) {
					...
					tmpProtolinfo = DLNADataProvider.queryDataByID(itemCharID,DLNADataProvider.UPNP_DMP_RES_PROTOCOLINFO,null);
					
					Log.v(TAG, "tmpProtolinfo is:"+tmpProtolinfo);
					if(tmpProtolinfo == null || !tmpProtolinfo.contains("DLNA.ORG_PN="))
					{
						showflag=false;
					}
					else if(tmpProtolinfo.contains("DLNA.ORG_PN=JPEG_SM")
							//||tmpProtolinfo.contains("DLNA.ORG_PN=JPEG_SM_ICO")
							||tmpProtolinfo.contains("DLNA.ORG_PN=JPEG_MED")
							||tmpProtolinfo.contains("DLNA.ORG_PN=JPEG_LRG")
							)
					{
						showflag = true;
					}
					...
					if(showflag == true)
					{
						DLNAFileInfo finfo = new DLNAFileInfo(i, FileName, itemCharID,
								...
								url,
								DLNADataProvider.queryDataByID(itemCharID,DLNADataProvider.UPNP_DMP_RES_DATE, null),
								0,0,null);
...
class DLNAFileInfoComparatorWithMode implements Comparator<DLNAFileInfo> {
	protected int mode;
	private int order=1;
	public DLNAFileInfoComparatorWithMode(int mode,int order) {
		this.mode = mode;
		this.order=order;
		// TODO Auto-generated constructor stub
	}
	public DLNAFileInfoComparatorWithMode(int mode) {
		this.mode = mode;
		// TODO Auto-generated constructor stub
	...
	public int compare(DLNAFileInfo object1, DLNAFileInfo object2) {
		...
		if(mode ==1 )
		{
			m1 = object1.getFileName();
			m2 = object2.getFileName();
		}
		else if (mode == 0)
		{
        ...
        else if(m1.compareToIgnoreCase(m2)>0)
        {
            result =-order;
        }
        else if(m1.compareToIgnoreCase(m2)<0)
