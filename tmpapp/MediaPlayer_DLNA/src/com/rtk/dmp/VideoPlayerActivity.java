...
public class VideoPlayerActivity extends Activity implements OnClickListener, OnTouchListener,OnGestureListener, Observer{
	...
	private boolean ablePlay = true;
	private boolean isActivityReady = false;
	private boolean canRecordResumePoint = true;
			...
			switch(position) {
				...
				case DETAILS_ID: {
					if(currIndex >= 0  && currIndex < fileArraySize) {
						...
						String date = DLNADataProvider.queryDataByID(
								fileCharIDArray.get(currIndex), DLNADataProvider.UPNP_DMP_RES_DATE, null);
					Log.d(TAG, "DATE=" + date);
					if(date == null || date == "") {
						date = "";
					} else { 
					...
					if(currIndex >= 0 && currIndex < fileArraySize) {
						String strTmp = DLNADataProvider.queryDataByID(
								fileCharIDArray.get(currIndex),
...
//				}
            ...
            case KeyEvent.KEYCODE_MENU : {
				Logger.v(tag, "MENU press");
                onAndOffBanner();
                return true;
			} 
			default :
				dealWithKeyCode(keyCode, event);
				break;
		}
...
//						}
						...
						boolean bDTCP = false;
						tmpProtolinfo = DLNADataProvider.queryDataByID(fileCharIDArray.get(currIndex),DLNADataProvider.UPNP_DMP_RES_PROTOCOLINFO,null);
					...
					public boolean onError(MediaPlayer mp, int what, int extra) {
						...
						switch(what) {
							...
							case MediaPlayer.MEDIA_ERROR_UNKNOWN: {
								...
								showMessageHint(VideoPlayerActivity.this.getResources().getString(R.string.msg_read_error));
			...
			public boolean setDataSource() {
				...
				if(currIndex < filePathArray.size())
	        	{
	        		...
	        		try {
						...
						ArrayList<String> protolinfoList = new ArrayList<String>(); 
						protolinfoList = DLNADataProvider.queryResourceListByIndex(fileCharIDArray.get(currIndex),DLNADataProvider.UPNP_DMP_RES_PROTOCOLINFO); 
						ArrayList<String> contentLenList = new ArrayList<String>(); 
						contentLenList = DLNADataProvider.queryResourceListByIndex(fileCharIDArray.get(currIndex),DLNADataProvider.UPNP_DMP_RES_SIZE); 
						ArrayList<String> durationList = new ArrayList<String>(); 
						durationList = DLNADataProvider.queryResourceListByIndex(fileCharIDArray.get(currIndex),DLNADataProvider.UPNP_DMP_RES_DURATION);
						ArrayList<String> bitRateList = new ArrayList<String>();
						bitRateList = DLNADataProvider.queryResourceListByIndex(fileCharIDArray.get(currIndex),DLNADataProvider.UPNP_DMP_RES_BITRATE);
						ablePlay = false; 
						for(int i = 0; i < protolinfoList.size(); i++){ 
							ablePlay = checkProtolInfo(protolinfoList.get(i));
							ablePlay = true;
							if(ablePlay){ 
								String tmpContentLength = null;
								if(contentLenList.size() > i) 
									tmpContentLength = contentLenList.get(i); 
								if(tmpContentLength != null) {
			        				tmpFilePath = tmpFilePath + " contentlength=" + tmpContentLength;
			        			}
			        			String tmpDuration = null;
			        			if(durationList.size() > i) 
									tmpDuration= durationList.get(i); 
								globalDuration = tmpDuration;
			        			if(tmpDuration != null) {
			        				tmpFilePath = tmpFilePath + " duration=" + tmpDuration;
			        			}
								String tmpBitRate = null;
								if(bitRateList.size() > i)
									tmpBitRate = bitRateList.get(i);
								if(tmpBitRate != null){
									tmpFilePath = tmpFilePath + " bitrate=" + tmpBitRate;
								}

								tmpFilePath = tmpFilePath + " supportMultistream fromDLNA";
								tmpProtolinfo = protolinfoList.get(i);
	...
	*/
	...
	//Support For KeyEvent
	private void dealWithKeyCode(int keyCode, KeyEvent event) {
		Log.v(TAG, "KeyPress & keyCode = " + keyCode);
		
		switch(keyCode) {
			case KeyEvent.KEYCODE_INFO: {	//165
				Log.v(TAG, "The key react by KeyEvent.KEYCODE_INFO & keyCode = " + keyCode);
				this.onClick(main_layout);
			} break;
			default: {
				showAndWaitToHideBanner();
			} break;
